...
class SRNN_model(object):
    ...
    def __init__(self, num_classes, num_frames, num_temp_features, num_st_features, num_units, max_gradient_norm,
        ...
        self.temp_features_names = ['face-face', 'belly-belly', 'rightElbow-rightElbow', 'leftElbow-leftElbow','rightArm-rightArm', 'leftArm-leftArm',
                                    'rightLeg-rightLeg', 'leftLeg-leftLeg']
        self.st_features_names = ['face-rightElbow','face-leftElbow',
                                  'face-leftArm', 'face-rightArm', 'face-belly',
                                  'rightElbow-rightArm','leftElbow-leftArm',
                                  'belly-leftArm', 'belly-rightArm', 'belly-rightElbow','belly-leftElbow',
                                  'belly-rightLeg', 'belly-leftLeg', 'rightArm-leftArm', 'rightLeg-leftLeg','rightElbow-leftElbow']
        nodes_names = {'face', 'elbow','arms', 'legs', 'belly'}
        ...
        weights = {'out': tf.Variable(tf.random_normal([num_units * num_frames * 5, num_classes]))}
        biases = {'out': tf.Variable(tf.random_normal([num_classes]))}

        # if num_layers == 1:
        #     #fullbodyRNN = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(num_units, state_is_tuple=True,reuse=tf.get_variable_scope().reuse,activation=tf.nn.softsign),output_keep_prob=1)
        #     fw_fullbodyRNN = tf.contrib.rnn.BasicLSTMCell(num_units, state_is_tuple=True,
        #                                                         reuse=tf.get_variable_scope().reuse,
        #                                                         activation=tf.nn.softsign)
        #     bw_fullbodyRNN = tf.contrib.rnn.BasicLSTMCell(num_units, state_is_tuple=True,
        #                                                         reuse=tf.get_variable_scope().reuse,
        #                                                         activation=tf.nn.softsign)
        #     #fullbodyRNN = tf.contrib.rnn.LSTMCell(num_units,initializer=tf.glorot_normal_initializer,state_is_tuple=True,reuse=tf.get_variable_scope().reuse,activation=tf.nn.softsign)
        # else:
        #     #fullbodyRNN = tf.contrib.rnn.MultiRNNCell([single_cell] * num_layers, state_is_tuple=True)
        #     cells = []
        #     for _ in range(num_layers):
        #         cell = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(num_units, state_is_tuple=True,reuse=tf.get_variable_scope().reuse,activation=tf.nn.softsign))
        #         cells.append(cell)
        #     fullbodyRNN = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.MultiRNNCell(cells,state_is_tuple=True))
        # # states['fullbody'] = fullbodyRNN.zero_state(self.batch_size, tf.float32)
        # states['fullbody_fw'] = fw_fullbodyRNN.zero_state(self.batch_size, tf.float32)
        # states['fullbody_bw'] = bw_fullbodyRNN.zero_state(self.batch_size, tf.float32)
        # infos['fullbody'] = {'input_gates': [], 'forget_gates': [], 'modulated_input_gates': [], 'output_gates': [],
        #                      'activations': [], 'state_c': [], 'state_m': []}
        ...
        with tf.variable_scope("SRNN"):
            for time_step in range(num_frames):
                ...
                node_inputs['face'] = tf.concat([outputs['face-face'], outputs['face-rightArm'], outputs['face-leftArm'], outputs['face-belly'],outputs['face-rightElbow'],outputs['face-leftElbow']], 1)


                node_inputs['elbow'] = tf.concat([outputs['rightElbow-rightElbow'],outputs['leftElbow-leftElbow'],outputs['rightElbow-leftElbow'],
                                                  outputs['face-rightElbow'],outputs['face-leftElbow'],outputs['rightElbow-rightArm'],outputs['leftElbow-leftArm'],
                                                  outputs['belly-rightElbow'],outputs['belly-leftElbow']],1)
                ...
                node_inputs['legs'] = tf.concat(
                    [outputs['rightLeg-rightLeg'], outputs['leftLeg-leftLeg'],
                ...
                fullbody_input = tf.concat(
                    [outputs['face'], outputs['belly'], outputs['elbow'],outputs['arms'],
                     outputs['legs']], 1)
