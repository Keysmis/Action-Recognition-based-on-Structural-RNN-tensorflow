...
class SRNN_model(object):
    ...
    def __init__(self, num_classes, num_frames, num_temp_features, num_st_features, num_units, max_gradient_norm,
        ...
        self.temp_features_names = ['face-face', 'belly-belly', 'rightHip-rightHip', 'leftHip-leftHip','rightArm-rightArm', 'leftArm-leftArm',
                                    'rightLeg-rightLeg', 'leftLeg-leftLeg']
        self.st_features_names =['face-leftArm', 'face-rightArm', 'face-belly',
                                  'belly-leftArm', 'belly-rightArm', 'belly-rightHip','belly-leftHip','rightHip-rightLeg','leftHip-leftLeg',
                                  'belly-rightLeg', 'belly-leftLeg', 'rightArm-leftArm', 'rightLeg-leftLeg','rightHip-leftHip']
        nodes_names = {'face','arms', 'hip','legs', 'belly'}
        edgesRNN = {}
        nodesRNN = {}
        states = {}
        infos = {}
        ...
        with tf.variable_scope("SRNN"):
            for time_step in range(num_frames):
                ...
                node_inputs['face'] = tf.concat([outputs['face-face'], outputs['face-rightArm'], outputs['face-leftArm'], outputs['face-belly']], 1)

                node_inputs['belly'] = tf.concat(
                    [outputs['belly-belly'], outputs['face-belly'],
                     outputs['belly-rightHip'],outputs['belly-leftHip'],
                     outputs['belly-leftArm'], outputs['belly-rightArm'],
                     outputs['belly-leftLeg'], outputs['belly-rightLeg']], 1)

                node_inputs['arms'] = tf.concat(
                    [outputs['rightArm-rightArm'], outputs['leftArm-leftArm'],outputs['face-rightArm'],
                   outputs['face-leftArm'], outputs['belly-rightArm'], outputs['belly-leftArm'],outputs['rightArm-leftArm']], 1)

                node_inputs['hip'] = tf.concat([outputs['rightHip-rightHip'],outputs['leftHip-leftHip'],outputs['rightHip-leftHip'],
                                                outputs['belly-rightHip'],outputs['belly-leftHip'],outputs['rightHip-rightLeg'],outputs['leftHip-leftLeg']], 1)


                node_inputs['legs'] = tf.concat(
                    [outputs['rightLeg-rightLeg'], outputs['leftLeg-leftLeg'],outputs['rightHip-rightLeg'],outputs['leftHip-leftLeg'],
                ...
                fullbody_input = tf.concat(
                    [outputs['face'], outputs['belly'], outputs['hip'],outputs['arms'],
                     outputs['legs']], 1)
